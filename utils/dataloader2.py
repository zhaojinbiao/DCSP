...
class MyDataset(Dataset):
    def __init__(self, opt, train):
        ...
        self.annotations = self.load_annotations(opt.train_annotation_path)
    ...
    def get_random_data(self, annotation_line, input_shape):
        ...
        if not self.train:
            ...
            if random.randint(0, 2) > 0:
                ...
                # 根据beta的值从offline生成的包加载所需的fog图
                    # train is true
                if self.train:
                    img_name = self.vocfog_traindata_dir + image_name  + '_' + ("%.2f" % beta) + '.' + image_name_index
                        
                
                else:
                    # val
                    img_name = self.vocfog_valdata_dir + image_name + '_' + ("%.2f" % beta) + '.' + image_name_index
                ...
                foggy_image, _ = image_preporcess(np.copy(foggy_image),input_shape,np.copy(box)) 
                image_data, box = image_preporcess(np.copy(image),input_shape,np.copy(box))
            # 1/3的概率载入clean作为训练数据
            else:
                if self.train: 
                    image, box = self.random_horizontal_flip(np.copy(image), np.copy(box))
                    image, box = self.random_crop(np.copy(image), np.copy(box))
                    image, box = self.random_translate(np.copy(image), np.copy(box))               
                image_data, box = image_preporcess(np.copy(image),input_shape,np.copy(box))
                # image_data = image_preporcess1(np.copy(image),input_shape)
                foggy_image =  image_data
            # image, box = self.random_horizontal_flip(np.copy(image), np.copy(box))
            # image, box = self.random_crop(np.copy(image), np.copy(box))
            # image, box = self.random_translate(np.copy(image), np.copy(box))               
