...
from utils.util_filters import lrelu, rgb2lum, tanh_range, lerp
import cv2
import math
import torch 
...
class DefogFilter(Filter):#Defog_param is in [Defog_range]#参数在这个范围之内
  ...
  def __init__(self, net, cfg):
    ...
    self.short_name = 'DF'
    self.begin_filter_parameter = cfg.defog_begin_param # 参数num: 1 posi: 0
    self.num_filter_parameters = 1#该filter的参数量

  def filter_param_regressor(self, features):
    return tanh_range(*self.cfg.defog_range)(features)
    # return torch.tanh(*self.cfg.defog_range)(features)
...
class ImprovedWhiteBalanceFilter(Filter):
  ...
  def __init__(self, net, cfg):
    ...
    self.short_name = 'W'
    self.channels = 3
    self.begin_filter_parameter = cfg.wb_begin_param # 参数num: 3 posi: 1-3
...
class GammaFilter(Filter):  #gamma_param is in [-gamma_range, gamma_range]
  ...
  def __init__(self, net, cfg):
    Filter.__init__(self, net, cfg)
    self.short_name = 'G'
    self.begin_filter_parameter = cfg.gamma_begin_param # 参数num: 1 posi: 4
...
class ToneFilter(Filter):
  ...
  def __init__(self, net, cfg):
    ...
    self.short_name = 'T'
    self.begin_filter_parameter = cfg.tone_begin_param # 参数num: 8 posi: 5-12
...
class ContrastFilter(Filter):
  ...
  def __init__(self, net, cfg):
    Filter.__init__(self, net, cfg)
    self.short_name = 'Ct'
    self.begin_filter_parameter = cfg.contrast_begin_param # 参数num: 1 posi: 13
...
class UsmFilter(Filter):#Usm_param is in [Defog_range]
  ...
  def __init__(self, net, cfg):
    Filter.__init__(self, net, cfg)
    self.short_name = 'UF'
    self.begin_filter_parameter = cfg.usm_begin_param # 参数列表的起始位置，14
    self.num_filter_parameters = 1
...
class ExposureFilter(Filter):
  ...
  def filter_param_regressor(self, features):#param is in (-self.cfg.exposure_range, self.cfg.exposure_range)
    # return tanh_range(*self.cfg.exposure_range)(features)
    return tanh_range(
