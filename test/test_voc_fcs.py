...
def parse_opt(known=False):
    ...
    parser.add_argument('--classes_path', type=str, default='datasets/classes/voc_fcs_classes.txt', help='分类标签路径')

    parser.add_argument('--model_path', type=str, default='train_weitht/last_epoch_weights.pth', help='加载模型路径') # 'pth/yolox_s.pth'


    parser.add_argument('--train_fog_annotation_path', type=str, default='datasets/FoggyCityspacetest/2007_train.txt', help='雾图训练集标注')
    parser.add_argument('--val_fog_annotation_path', type=str, default='datasets/FoggyCityspacetest/2007_train.txt', help='雾图验证集标注')
    parser.add_argument('--train_clean_annotation_path', type=str, default='datasets/FoggyCityspacetest/2007_train.txt', help='干净图训练集标注')
    parser.add_argument('--val_clean_annotation_path', type=str, default='datasets/FoggyCityspacetest/2007_train.txt', help='干净图验证集标注')

    parser.add_argument('--vocfog_traindata_dir', dest='vocfog_traindata_dir', default='datasets/FoggyCityspacetest/VOC2007/JPEGImages/',help='train the dir contains ten levels synthetic foggy images')
    parser.add_argument('--vocfog_valdata_dir', dest='vocfog_valdata_dir', default='datasets/FoggyCityspacetest/VOC2007/JPEGImages/',help='雾图路径')
    ...
    #-----------------------------------------------------------------------------------------------------------------------------------------------------
    parser.add_argument('--test_out_path', type=str, default='datasets/FoggyCityspacetest/test_out', help='推理map的路径')
...
#--------------------------------------------------------------------tiao zheng model mood----------------------------------------------------------------------------------
    ...
    for iteration, batch in enumerate(gen_test):
            ...
            if opt.detection:
                    ...
                    results = non_max_suppression(outputs, num_classes, opt.input_shape, image_shape, letterbox_image = True, conf_thres = 0.0001, nms_thres = 0.5)
                    ...
                    #-----------------------------------------------------------------------------------------------------------------------------------
                    root = ET.parse(os.path.join("datasets/FoggyCityspacetest/VOC2007", "Annotations/"+image_id+".xml")).getroot()
