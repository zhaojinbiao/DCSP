...
def parse_opt(known=False):
    ...
    parser.add_argument('--train_fog_annotation_path', type=str, default='voc-fog/test/2007_test_fog.txt', help='雾图训练集标注')
    parser.add_argument('--val_fog_annotation_path', type=str, default='voc-fog/test/2007_test_fog.txt', help='雾图验证集标注')
    parser.add_argument('--train_clean_annotation_path', type=str, default='voc-fog/test/2007_test_fog.txt', help='干净图训练集标注')
    parser.add_argument('--val_clean_annotation_path', type=str, default='voc-fog/test/2007_test.txt', help='干净图验证集标注')
    ...
    parser.add_argument('--vocfog_traindata_dir', dest='vocfog_traindata_dir', default='voc-fog/test/VOC2007/VOCtest-FOG/',help='train the dir contains ten levels synthetic foggy images')
    parser.add_argument('--vocfog_valdata_dir', dest='vocfog_valdata_dir', default='voc-fog/test/VOC2007/VOCtest-FOG/',help='雾图路径')
    ...
    parser.add_argument('--test_out_path', type=str, default='voc-fog/test/test_out', help='推理map的路径')
...
#--------------------------------------------------------------------tiao zheng model mood----------------------------------------------------------------------------------
    ...
    for iteration, batch in enumerate(gen_test):
            ...
            if opt.psnr:
                ...
                f.close()
            if opt.detection:
                    d = open(os.path.join("voc-fog/test/test_out/", "detection-results/" + image_id +".txt"),"w")
                    # 解码
                    outputs = outputs_foggy_iamges_label
                    outputs = decode_outputs(outputs, opt.input_shape)
                    results = non_max_suppression(outputs, num_classes, opt.input_shape, image_shape, letterbox_image = True, conf_thres = 0.0001, nms_thres = 0.545)
                    ...
                    #  获取GT
                    d_gt = open(os.path.join('voc-fog/test/test_out/', "ground-truth/"+image_id+".txt"), "w")
                    root = ET.parse(os.path.join("voc-fog/test/VOC2007", "Annotations/"+image_id+".xml")).getroot()
    ...
    if opt.psnr:
        ...
        f.close()
    print('Successfully save results to: ' + "voc-fog/test/test_out/psnr_results/psnr.txt")
    MINOVERLAP = [0.5]
    if opt.detection:
        # 计算map
        for i in MINOVERLAP:
